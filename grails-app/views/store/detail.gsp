...
<html>
	...
	<p class="title">杂货 >> ${store.title}</p>
	</g:if>
	<g:if test="${conts.size()==1}">
	<h1>${store.title}</h1>
	</g:if>
	...
	<g:if test="${index==0}">
	<g:if test="${contid>1}">
	<li><a href="${createLink(controller:'store', action:'detail', params:[sutid:cont.sutid,contid:contid-1,storetype:storetype])}">
	...
	<li><a href="${createLink(controller:'store', action:'detail', params:[sutid:cont.sutid,contid:cont.contid,storetype:storetype])}">
	${cont.title}</a></li>
	</g:if>
	<g:if test="${contid==cont.contid}">
